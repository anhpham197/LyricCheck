...
<main class="sm:mx-auto sm:mt-5">
    <div class="lg:px-64">
        ...
        <div class="grid grid-cols-3 gap-4">
            <form action="{{ route('savePost') }}" method="post" id="save-form" class="w-full col-span-2" enctype="multipart/form-data"> @csrf
                <input type="text" name="title" id="title" class="w-full form-input mb-3 text-white" style="background: #181A1B; border: 1px solid #4f4f4f" value="{{ $title }}">
                {{-- <textarea name="content" id="content" rows="20" style="background: #1E2122;" class="w-full pl-3 pt-3 rounded-md text-white leading-8" readonly>{{!! $content !!}}</textarea> --}}
                <div name="content" id="content" rows="20" style="background: #1E2122;" class="w-full pl-3 pt-3 rounded-md text-white leading-8" readonly>{!! $content !!}</div>
            </form>
                ...
                <div style="background: #1E2122">
                    <div class="py-4 px-2 flex gap-2 flex-wrap">
                        @foreach ($checkedWords as $word)                
                            <div class="px-2 py-1 rounded text-white" style="background: #732828">{{ $word }}</div>
                        @endforeach
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        saveForm.addEventListener('submit', function (e) { 
            ...
            xhr.open(saveForm.method, saveForm.action, true)
