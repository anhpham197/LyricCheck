...
@section('content')
    <main class="sm:container sm:mx-auto sm:mt-5">
        <div class="lg:pl-64 pr-64">
            <div class="flex gap-4 text-white font-semibold my-7">
                <div>
                ...
                <div class="flex justify-center items-center font-medium py-2 px-4 bg-white rounded-md text-red-700 bg-red-100 border border-red-300"
                    ...
                    <div class="font-normal max-w-full flex-initial">Xóa thành công</div>
                </div>
            </div>

            <div class="relative flex w-full flex-wrap items-stretch mb-7">
                <span
                ...
                <input type="search" id="search" name="search" class="form-input placeholder-gray-400 w-72 text-white pl-10"
                    placeholder="Tìm kiếm..."
                    style="font-family: 'Font Awesome 5 Free', 'system-ui'; background: #181A1B; border: 1px solid #4f4f4f">
            </div>

            <div class="grid lg:grid-cols-2 xl:grid-cols-4 gap-3" id="data">
                @foreach ($posts as $post)
                    <div style="background: #1e2122" class="px-4 py-3 rounded-md">
                        <div class="flex justify-between mb-1 items-start">
                            <input type="text" hidden name="post" value="{{ $post->id }}" disabled>
                            <a href="{{ route('showPost', ['id' => $post->id]) }}"
                                class="text-white font-medium leading-snug max-lines">{{ $post->title }}</a>
                            <button class="pt-1 delete-post" data-id="{{ $post->id }}">
                        ...
                        <div class="mb-2">
                            @if ($post->error == 0)
                                <span style="color: #058BFF">{{ $post->error }} từ vi phạm</span>
                            @else
                                <span style="color: #EB5757">{{ $post->error }} từ vi phạm</span>
                            @endif
                        </div>
            ...
            <script type="text/javascript">
                $("#data").on('click', '.delete-post', function() {
                    var id = $(this).data("id");
                ...
                $('#search').on('keyup', function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            <div style="background: #1e2122" class="px-4 py-3 rounded-md">
                                <div class="flex justify-between mb-1 items-start">
                                    <input type="text" hidden name="post" value="${item.id}" disabled>
                                    <a href="/show/${item.id}" class="text-white font-medium leading-snug max-lines">${item.title}</a>
                                    <button class="pt-1 delete-post" data-id="${item.id}">
                                ...
                                <div class="mb-2">
                                    <span style="color: ${item.error===0 ?'#058bff' : '#eb5757'}">${item.error} từ vi phạm</span>
                                </div>
                                <div style="color: #828282">
                                    <span>${time}</span>
                                    <span>&bull;</span>
